...
namespace NuGetGallery.Helpers
{
    ...
    {
        public class TheCreateDisplayPackageViewModelMethod : ViewModelHelperFactsBase
        {
            ...
            public void ItDeterminesRepositoryKind(string repoUrl, string repoType, RepositoryKind expectedKind, string expectedUrl)
            {
                ...
                var model = _target.CreateDisplayPackageViewModel(package, currentUser: null, deprecation: null, readmeHtml: null);
            ...
            public void LicenseNamesAreParsedByCommas()
            {
                ...
                var packageViewModel = _target.CreateDisplayPackageViewModel(package, currentUser: null, deprecation: null, readmeHtml: null);
            ...
            public void TheCtorSortsPackageVersionsProperly()
            {
                ...
                var packageVersions = _target.CreateDisplayPackageViewModel(package, currentUser: null, deprecation: null, readmeHtml: null)
            ...
            public void TheCtorDoesNotPopulateLatestSymbolsPackageForHistory()
            {
                ...
                var viewModel = _target.CreateDisplayPackageViewModel(package, currentUser: null, deprecation: null, readmeHtml: null);
            ...
            public static IEnumerable<object[]> SetsBasicMetadataProperly_Data =>
                from isDeprecationEnabled in new[] { false, true }
                from isLatestSemVer2 in new[] { false, true }
                from isAtomFeedEnabled in new[] { false, true }
                select new object[] { isDeprecationEnabled, isLatestSemVer2, isAtomFeedEnabled };

            [Theory]
            [MemberData(nameof(SetsBasicMetadataProperly_Data))]
            public void SetsBasicMetadataProperly(bool isDeprecationEnabled, bool isLatestSemVer2, bool isAtomFeedEnabled)
            {
                const string packageId = "someid";
                const string packageVersion = "1.2.3";
                const string normalizedVersion = "5.6.7";
                var package = new Package
                {
                    PackageRegistration = new PackageRegistration
                    {
                ...
                _validationServiceMock
                    .Setup(v => v.GetLatestPackageValidationIssues(package))
                    .Returns(expectedIssues);

                var user = new User();
                _featureFlagServiceMock
                    .Setup(ffs => ffs.IsManageDeprecationEnabled(user, package.PackageRegistration))
                ...
                string readmeHtml = "somereadme";

                var viewModel = _target.CreateDisplayPackageViewModel(package, user, deprecation, readmeHtml);

                Assert.Equal(packageId, viewModel.Id);
                Assert.Equal(normalizedVersion, viewModel.Version);
                Assert.Same(readmeHtml, viewModel.ReadMeHtml);
                Assert.Equal(expectedIssues, viewModel.PackageValidationIssues);
                _featureFlagServiceMock.Verify();
                Assert.Equal(isDeprecationEnabled, viewModel.IsPackageDeprecationEnabled);
                Assert.Equal(isLatestSemVer2, viewModel.LatestVersionSemVer2);
                Assert.Equal(isAtomFeedEnabled, viewModel.IsAtomFeedEnabled);
            }

            [Fact]
            public void SplitsLicenseExpressionWhenProvided()
            {
                const string expression = "some expression";

                var segments = new List<CompositeLicenseExpressionSegment>();
                _licenseExpressionSplitterMock
                ...
                var user = new User();
                var viewModel = _target.CreateDisplayPackageViewModel(package, user, deprecation: null, readmeHtml: null);

                _licenseExpressionSplitterMock
                    .Verify(les => les.SplitExpression(It.IsAny<string>()), Times.Once);

                Assert.Same(segments, viewModel.LicenseExpressionSegments);
            }
            ...
            public class ThePushedByField : ViewModelHelperFactsBase
            {
                ...
                {
                    var model = _target.CreateDisplayPackageViewModel(package, currentUser: currentUser, deprecation: null, readmeHtml: null);
                ...
                bool hasAlternatePackage)
            {
                ...
                // Act
                var model = _target.CreateDisplayPackageViewModel(package, currentUser: null, deprecation: deprecation, readmeHtml: null);
